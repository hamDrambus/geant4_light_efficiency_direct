...
CustomParticleChangeForTransport::CustomParticleChangeForTransport()
...
{
	if (verboseLevel>2) {
		G4cout << "CustomParticleChangeForTransport::CustomParticleChangeForTransport() "
			<< G4endl;
	}
}
...
CustomParticleChangeForTransport(const CustomParticleChangeForTransport &r)
: G4ParticleChange(r),
fpVectorOfAuxiliaryPointsPointer(0)
{
	if (verboseLevel>0) {
		G4cout << "CustomParticleChangeForTransport::  copy constructor is called "
			<< G4endl;
	}
...
// assignemnt operator
CustomParticleChangeForTransport &
CustomParticleChangeForTransport::operator=(const CustomParticleChangeForTransport &r)
{
	if (verboseLevel>1) {
		G4cout << "CustomParticleChangeForTransport:: assignment operator is called "
			<< G4endl;
	}
	if (this != &r)
	{
...
G4Step* CustomParticleChangeForTransport::UpdateStepForAtRest(G4Step* pStep)
{
	// Nothing happens for AtRestDoIt
	if (verboseLevel>0) {
		G4cout << "CustomParticleChangeForTransport::UpdateStepForAtRest() is called"
			<< G4endl;
...
G4Step* CustomParticleChangeForTransport::UpdateStepForAlongStep(G4Step* pStep)
{
...
G4Step* CustomParticleChangeForTransport::UpdateStepForPostStep(G4Step* pStep)
{
...
void CustomParticleChangeForTransport::DumpInfo() const
{

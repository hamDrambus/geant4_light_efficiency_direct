...
// =======================================================================
// Modified:   
//	 02 Feb  2017, Geffdroid: added fCurrentTouchable = track->... in AlongStepGPIL() 
//					in order to work with processes that change touchable
...
#include "CustomTransportationProcessType.hh"
...
CustomTransportation::CustomTransportation(G4int verbosity)
	...
	fVerboseLevel(verbosity)
{
	...
	if (fVerboseLevel > 0)
	{
		G4cout << " CustomTransportation constructor> set fShortStepOptimisation to ";
...
CustomTransportation::~CustomTransportation()
{
	...
	*selection = CandidateForSelection;
		...
		else
		{
			...
			if (fVerboseLevel > 1)
			{
				if (std::fabs(startEnergy - endEnergy) > perThousand * endEnergy)
				{
					...
					{
						no_warnings++;
						G4cout << "WARNING - CustomTransportation::AlongStepGetPIL() "
						...
						G4cerr << "ERROR - CustomTransportation::AlongStepGetPIL()" << G4endl
	...
	if (currentSafety < fEndPointDistance)
	{
		if (particleCharge != 0.0)
		{
			...
			G4cout.precision(12);
			G4cout << "***CustomTransportation::AlongStepGPIL ** " << G4endl;
...
G4VParticleChange* CustomTransportation::AlongStepDoIt(const G4Track& track,
	const G4Step&  stepData)
{
	...
	if (fParticleIsLooping)
	{
			...
			|| (fNoLooperTrials >= fThresholdTrials))
		{
			...
			{
				G4cout << " CustomTransportation is killing track that is looping or stuck "
					<< G4endl
		...
		else
		{
			...
			if ((fVerboseLevel > 2))
			{
				G4cout << "   CustomTransportation::AlongStepDoIt(): Particle looping -  "
...
G4VParticleChange* CustomTransportation::PostStepDoIt(const G4Track& track,
	const G4Step&)
{
...
// New method takes over the responsibility to reset the state of CustomTransportation
//   object at the start of a new track or the resumption of a suspended track. 

void
CustomTransportation::StartTracking(G4Track* aTrack)
{
